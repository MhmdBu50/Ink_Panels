...
if(isset($_SESSION['cart_message'])) {
    echo '<div class="alert">' . $_SESSION['cart_message'] . '</div>';
...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        $stmt = $db->prepare("SELECT sc.*, mc.title, mc.cover_image, mc.price as unit_price
                             FROM shopping_cart sc
                             JOIN manga_comic mc ON sc.MC_ID = mc.MC_ID
                             WHERE sc.user_id = ?");
        ...
        $shipping_fee = 132.90; // Your fixed shipping fee
...
<body>
    ...
    <div class="productsmainbox"><p class="text1">Shopping Cart</p><?php if (!empty($cart_items)): ?><br><br>
        ...
        <p style="color: white">You have 6 items in your cart</p>
        <div class="submain">
        <?php foreach($cart_items as $item):
            ...
            ?>
            <div class ="itembox">
                    <img src="data:<?php $mime_type?>;base64,<?php base64_encode($item['cover_image'])?>">

                                <p class="title"><?php  $item['title'] ?></p>
                    <div class="descbox"><p><?php $item['description'] ?></p></div>
                    ...
                    <div class="price">
                        ...
                        </svg><?php number_format($item['price'],2)?>
                    </div>

                        <form method="post">
                            <div class="doradd"> 
                                 <input type="hidden" name="cart_id" value="<?= $item['cart_id'] ?>">
                                <div><button class="doraddbuttons"    onclick="decrement()">-</button></div>
                                <input type="number" name="quantity" value="<?= $item['quantity'] ?>" min="1">
                                <span id="value">1</span>
                                <div ><button class="doraddbuttons" onclick="increment()">+</button></div>
...
<path d="M17.7092 32.3882L17.6471 33L28.9685 30.6507L29.1674 30.198L29.3538 29.733L29.6023 28.9622L29.7639 28.3259L29.9006 27.6162L30 26.8454L18.6661 29.2191L18.5543 29.4883L18.2063 30.3815L17.9702 31.1524L17.8086 31.8009L17.7092 32.3882Z" fill="black"/>
            ...
            Total: <?= number_format($cart_summary['total'], 2) ?>
        </div>
